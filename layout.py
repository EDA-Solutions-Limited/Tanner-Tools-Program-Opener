...
class Layout:
    ...
    def generate_layout(self):
        ...
        if len(self.list) <= 19:  # If everything fits in one screen, hide the scroll bars
            scrollable_flag = False
        elif 19 < len(self.list) <= 51:   # item_count > 19 and item_count <= 37
        ...
        for i in range(1, len(self.list), 2):
            if i+1 <= len(self.list)-1:
                col += [[self.generate_button(self.list[i]),
                        self.generate_button(self.list[i+1])]]
            else:
                col += [[self.generate_button(self.list[i]), self.generate_button("")]]
        if "open program" not in self.list:
            top = [[sg.Button("Back", size=(5, 1), font=('Franklin Gothic Book', 12), button_color=("black", "#84848a"), visible=True),
                    sg.Text(self.text, size=(34, 1), justification='right', background_color="#272533",  # Layer 2:  self.text question (variable) and back button
                            text_color='white', font=('Franklin Gothic Book', 12))]]
        else:
            top = [[sg.Text(self.text, size=(34, 1), justification='right', background_color="#272533",  # Layer 2:  self.text question (variable) and back button
                            text_color='white', font=('Franklin Gothic Book', 12))]]
    ...
    def generate_button(self,stuffinlist):  # Adds the text to be displayed on the button (Which is also the value of the button), and gives it properties (size, visibility..)
        # #-----DEFAULT SETTINGS--------------------------##
